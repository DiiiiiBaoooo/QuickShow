...
//api add new show to db
export const addShow = async (req, res) => {
    try {
        const { movieId, showsInput, showPrice } = req.body;

        // Validate required fields
        if (!movieId || !showsInput || showPrice === undefined) {
        ...
        for (const show of showsInput) {
            // Validate show.date and show.time
            if (!show.date || !Array.isArray(show.time)) {
                return res.status(400).json({
                    success: false,
                    message: "Each show must have a valid date and time array"
                });
            }
...
// api to ghet all shows from the db
